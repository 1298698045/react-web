...
export function getTimeDistance ( type ) {
	...
	if ( type === 'lastMonth' ) {
		...
		return [moment( start.getTime() ) ,  moment( end )];
	}
	if ( type === 'yesterday' ) {
		now.setHours( 0 );
		now.setMinutes( 0 );
		...
		return [moment( date2.getTime() ) ,  moment( date2.getTime() )];
	}
	if ( type === 'day_7' ) {
		now.setHours( 0 );
		now.setMinutes( 0 );
		now.setSeconds( 0 );
		let date2 = new Date(now);
		date2.setDate(now.getDate() - 7);
		return [moment( date2.getTime() ) ,  moment( now )];
	}
	if ( type === 'day_30' ) {
		now.setHours( 0 );
		now.setMinutes( 0 );
