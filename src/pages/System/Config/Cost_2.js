	...
	const tableColumns = [
		...
		},
		{
			...
			customRender: ( text, record, index ) => (
				<Privilege privs={[ 'supply_fee_setting' ]} noMatch={'设置'}>
				<a key="setting" onClick={() => {
					let costEdited = record.amount;
					Modal.confirm( {
						title: '补考费标准 - ' + getDictValue(dictionary, 'license_type', record.licenseType) + ' - ' + getDictValue(dictionary, 'km', record.kmCode),
						content: <Input placeholder="补考费" defaultValue={costEdited}
