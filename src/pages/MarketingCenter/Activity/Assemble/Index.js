...
const Activity = props => {
	...
	// 表格-字段
	const tableColumns = [ ...FIELDS.ACTIVITY.ASSEMBLE_TABLE_HEAD ]
	let tj_id = null;
	tableColumns.push( {
		title: '操作',
		key: 'actions',
		...
		customRender: ( text, record, ) => {
			...
			return <Buttons>
				<Privilege privs={[ 'publish_group_buy' ]} key="publish_group_buy">
					{
				...
				<Privilege privs={[ 'adjust_group_buy' ]} key="adjust_group_buy">
				...
				<Privilege privs={[ 'copy_group_buy' ]} key="copy_group_buy">
					{String( status ) === 'END' ?
						<a key="a5" onClick={e => {
							e.stopPropagation();
							if ( !getActivityInfoLoading ) {
				...
				<Privilege privs={[ 'end_group_buy' ]} key="end_group_buy">
				...
				<Privilege privs={[ 'statistics_group_buy' ]} key="statistics_group_buy">
				...
				<Privilege privs={[ 'group_detail_group_buy' ]} key="group_detail_group_buy">
					{String( status ) === 'PROGRESS' || String( status ) === 'END' ?
						<a key="a8" onClick={e => {
							e.stopPropagation();
							setSelectItem( {
						...
						<Spin size="small"/>} 参团详情</a> : <Text disabled>参团详情</Text>}
				</Privilege>
				
				<Privilege privs={[ 'activity_detail_group_buy' ]} key="activity_detail_group_buy">
					{<a key="a9" onClick={e => {
						e.stopPropagation();
						if ( !getActivityInfoLoading ) {
	...
	const tableActions = [
		...
		<Privilege privs={['promotion_check']} key="promotion_check">
			<Button
