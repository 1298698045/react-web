...
const SearchForm = props => {
	const { form, dispatch, loading,dictionary, onSearch, tab } = props;
	const [ getLicenseType, setLicenseType ] = useGetSet( '-1' );
	const [ getIntroducerType, setIntroducerType ] = useGetSet( '-1' );
	const [ getIncludeWaitPay, setIncludeWaitPay ] = useGetSet( '0' );
	const [ getIncludeLeaveSchool, setIncludeLeaveSchool ] = useGetSet( '0' );
	...
	return (
		<Form>
			<Row>
				...
				{tab !== '3' && tab !== '4' && <Col  span={8}>
					<Form.Item label="申领类型" style={{display: 'flex', marginBottom: 0}} >
						<Select allowClear
							mode="tags"
							placeholder="请选择"
							onChange={value => search('licenseType', value.join(','))}
							style={{ width: 180 }}
						>
							{dictionary.license_type ? dictionary.license_type.map(one => <Option key={one.dKey}>{one.dValue}</Option>) : ''}
						</Select>
					</Form.Item>
				</Col>}
				{tab === '1' && <Col  span={8}>
					<Form.Item label="介绍人归属" style={{display: 'flex', marginBottom: 0}}>
						<Select allowClear
							placeholder="请选择"
							onChange={value => search('introducerType', value)}
							style={{ width: 180 }}
						>
							<Option key='1'>员工</Option>
							<Option key='2'>校外</Option>
						</Select>
					</Form.Item>
				</Col>}
				<Col  span={4} >
					<Form.Item style={{display: 'flex', marginBottom: 0}} ><Checkbox onChange={e => search('includeWaitPay', e.target.checked ? '1' : '0')}>按报名时间统计</Checkbox></Form.Item>
				</Col>
				<Col  span={4}>
					<Form.Item style={{display: 'flex', marginBottom: 0}} ><Checkbox onChange={e => search('includeLeaveSchool',  e.target.checked ? '1' : '0')}>含已退学</Checkbox></Form.Item>
				</Col>
			</Row>
		</Form>);
};
