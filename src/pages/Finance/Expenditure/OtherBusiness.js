...
import { useGetSet, useToggle,useEffectOnce } from 'react-use';
...
const tableName = 'financeExpenditureOtherBusinessList';

const OtherBusiness = props => {
	...
	const originColumns = [
		...
		},
		{
			...
			customRender: ( text, record, ) => {
				...
				return (
					<Privilege privs={[ 'bussiness_expend_detail' ]}  noMatch={getDictValue( dictionary, 'outlay_type_other', type )}>
					<a onClick={e => {
						e.stopPropagation();
						let item = record
					...
					}}>{getDictValue( dictionary, 'outlay_type_other', type )}</a></Privilege>)
			}
		},
		{
		...
		FIELDS.STUDENT.MEMO,
		{
			...
			customRender: ( text, record, ) => {
				return (
					<Privilege privs={[ 'other_bussiness_expend_delete' ]}  noMatch={'删除'}>
						<Popconfirm
							title="是否确认删除这笔支出记录？"
							onConfirm={e => {
								e.stopPropagation();
	...
	const tableActions = [
		<Privilege privs={[ 'other_bussiness_expend_add' ]}  key={'other_bussiness_expend_add'}>
		<Button key="add-button"
			icon="plus"
			htmlType="button"
			type="primary"
			onClick={() => toggleAddFeePopVisible( true )}>新增</Button></Privilege>,
		<Privilege privs={[ 'other_bussiness_expend_export' ]}  key={'other_bussiness_expend_export'}>
			...
			<WithTableName
				...
				formFields={[
					...
					<WrapperComplexFormItem
						...
						values={dictionary[ 'outlay_type_other' ] ? dictionary[ 'outlay_type_other' ] : []}
						onChange={onChange}
						form={form}
					/>,
					<WrapperComplexFormItem
