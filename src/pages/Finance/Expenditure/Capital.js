...
const tableName = 'financeExpenditureCapitalList';
...
const Capital = props => {
	...
	const originColumns = [
		...
		},
		{
			...
			customRender: ( text, record, ) => {
				...
				return (
				<Privilege privs={[ 'fund_expend_detail' ]}  noMatch={getDictValue( dictionary, 'outlay_type_capital', type )}>
				<a onClick={e => {
					e.stopPropagation();
					let item = record
				...
				}}>{getDictValue( dictionary, 'outlay_type_capital', type )}</a></Privilege>)
			}
		},
		{
		...
		FIELDS.STUDENT.MEMO,
		{
			...
			customRender: ( text, record, ) => {
				return (
				<Privilege privs={[ 'fund_expend_delete' ]}  noMatch={'删除'}>
					<Popconfirm
						title="是否确认删除这笔支出记录？"
						onConfirm={e => {
							e.stopPropagation();
	...
	const tableActions = [
		<Privilege privs={[ 'fund_expend_add' ]}  key={'fund_expend_add'}>
		<Button key="add-button"
			icon="plus"
			htmlType="button"
			type="primary"
			onClick={() => toggleAddFeePopVisible( true )}>新增</Button></Privilege>,
		<Privilege privs={[ 'fund_expend_export' ]}  key={'fund_expend_export'}>
			...
			<WithTableName
				...
				formFields={[
					...
					<WrapperComplexFormItem
						...
						values={dictionary[ 'outlay_type_capital' ] ? dictionary[ 'outlay_type_capital' ] : []}
						onChange={onChange}
						form={form}
					/>,
					<WrapperComplexFormItem
