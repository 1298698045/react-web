...
const tableName = 'financeExpenditureIncomeTaxList';
...
const IncomeTax = props => {
	...
	const originColumns = [
		...
		},
		{
			...
			customRender: ( text, record, ) => {
				...
				return (
					<Privilege privs={[ 'tax_detail' ]}  noMatch={getDictValue( dictionary, 'outlay_type_tax', type )}>
					<a onClick={e => {
						e.stopPropagation();
						let item = record
					...
					}}>{getDictValue( dictionary, 'outlay_type_tax', type )}</a></Privilege>)
			}
		},
		{
		...
		FIELDS.STUDENT.MEMO,
		{
			...
			customRender: ( text, record, ) => {
				return (
					<Privilege privs={[ 'tax_delete' ]}  noMatch={'删除'}>
						<Popconfirm
							title="是否确认删除这笔支出记录？"
							onConfirm={e => {
								e.stopPropagation();
	...
	const tableActions = [
		<Privilege privs={[ 'tax_add' ]}  key={'tax_add'}>
		<Button key="add-button"
			icon="plus"
			htmlType="button"
			type="primary"
			onClick={() => toggleAddFeePopVisible( true )}>新增</Button></Privilege>,
		<Privilege privs={[ 'tax_export' ]}  key={'tax_export'}>
			...
			<WithTableName
				...
				formFields={[
					...
					<WrapperComplexFormItem
						...
						values={dictionary[ 'outlay_type_tax' ] ? dictionary[ 'outlay_type_tax' ] : []}
						onChange={onChange}
						form={form}
					/>,
					<WrapperComplexFormItem
