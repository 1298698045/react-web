...
const tableName = 'financeExpenditureMainBusinessList';
...
const MainBusiness = props => {
	...
	useEffectOnce( () => {
		...
		queryDictionary( dispatch, 'site_id' );
	...
	const originColumns = [
		...
		},
		{
			...
			customRender: ( text, record, ) => {
				...
				return (
					<Privilege privs={[ 'bussiness_expend_detail' ]}  noMatch={getDictValue( dictionary, 'outlay_type_main', type )}>
					<a onClick={e => {
						e.stopPropagation();
						let item = record
					...
					}}>{getDictValue( dictionary, 'outlay_type_main', type )}</a></Privilege>)
			}
		},
		{
		...
		FIELDS.STUDENT.MEMO,
		{
			...
			customRender: ( text, record, ) => {
				return (
					<Privilege privs={[ 'bussiness_expend_delete' ]}  noMatch={'删除'}>
						<Popconfirm
							title="是否确认删除这笔支出记录？"
							onConfirm={e => {
								e.stopPropagation();
	...
	const tableActions = [
		<Privilege privs={[ 'bussiness_expend_add' ]}  key={'bussiness_expend_add'}>
		<Button key="add-button"
			icon="plus"
			htmlType="button"
			type="primary"
			onClick={() => toggleAddFeePopVisible( true )}>新增</Button></Privilege>,
		<Privilege privs={[ 'bussiness_expend_export' ]}  key={'bussiness_expend_export'}>
			...
			<WithTableName
				...
				formFields={[
					...
					<WrapperComplexFormItem
						...
						values={dictionary[ 'outlay_type_main' ] ? dictionary[ 'outlay_type_main' ] : []}
						onChange={onChange}
						form={form}
					/>,
					<WrapperComplexFormItem
